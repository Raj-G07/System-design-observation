...
!!! note ""
    ...
    It acts as an **intermediary** between a client and a real object — useful for controlling creation, access, or communication with that object.

    > 🧠 In short: The proxy **wraps** another object to **add behavior** (like security, lazy loading, or remote access) without changing the real object’s code.
...
You don’t directly use cash — the card handles communication with the bank (real subject) and ensures proper authorization.
...
- **RealSubject** → The real object doing the main work  
- **Proxy** → Controls access to RealSubject (can add caching, access control, remote handling, etc.)  
...
Used for lazy loading — creates expensive objects only when needed.
...
???+ link 
     ...
     - [GeeksforGeeks – Proxy Design Pattern in C++](https://www.geeksforgeeks.org/system-design/proxy-design-pattern/)
