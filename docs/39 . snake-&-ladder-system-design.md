...
- **Maintains:**
  ...
  - **size** → scalable board dimension.
  - **snakes** → mapping from snake head to tail.
  - **ladders** → mapping from ladder bottom to top.

- Uses a strategy to initialize the layout (snakes and ladders).
- Provides getNextPosition() method to determine a player’s new position after movement.
...
- Responsible for generating random moves (1–6).
...
public:
    ...
    int getNextPosition(int pos) {
        if (snakes.count(pos)) {
            cout << "Oops! Snake bite at " << pos << " → " << snakes[pos] << "\n";
            return snakes[pos];
        }
        if (ladders.count(pos)) {
            cout << "Yay! Ladder at " << pos << " → " << ladders[pos] << "\n";
            return ladders[pos];
        }
        return pos;
    }
