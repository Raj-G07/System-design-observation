...
It’s especially useful when an application uses a large number of similar objects that consume considerable memory.
...
Each character doesn’t need its own font, color, or style object — these can be shared among similar characters.  
...
| Type | Meaning |
|------|----------|
| **Intrinsic State** | Shared state — independent of context (e.g., character shape, font type) |
| **Extrinsic State** | Context-specific state — depends on context (e.g., character position, color) |
...
- **Intrinsic State Sharing** — Core of the pattern (shared properties).

- **Extrinsic State Passing** — Passed from client at runtime.

- **Factory-Based Management** — Centralized object reuse.
