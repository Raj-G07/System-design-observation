...
**1.Dynamic Coupon Addition** – New coupons can be registered at runtime without restarting the system.
...
**3.Supports Flat and Percentage Discounts** – Discounts can either be a fixed amount or a percentage of the total.

**4.Stackable Control** – Some coupons can be applied together, while others cannot.

**5.Thread Safety** – The coupon registry must handle concurrent coupon registrations and lookups safely.

**6.Extensible Design** – New coupon types or rules can be added easily by extending the base coupon interface.
...
`isApplicable(Cart)` → Checks if coupon conditions are met.

`applyDiscount(Cart)` → Applies discount logic.

`isStackable()` → Determines if it can be combined with others.

**Attributes:**

`id` – Unique identifier for the coupon.

`type` – Type of the coupon (Seasonal, Loyalty, etc.).

`stackable` – Whether the coupon can be used with others.
...
- Applies a fixed discount (e.g., ₹100 off).
...
- `totalAmount` – Running total after discount application.
...
**Design Note:**
This allows administrators or systems to add new coupons while the system is live — enabling runtime configurability.

---

## 🧠 Design Principles Used

- **Open/Closed Principle** → New coupons can be added without modifying existing code.
- **Single Responsibility Principle** → Each class handles one concern (e.g., `CouponManager` applies coupons, `Registry` manages storage).
- **Thread Safety** → Mutex locking in `ThreadSafeCouponRegistry` ensures safe concurrent access.
- **Extensibility** → Base `Coupon` interface supports future coupon types (e.g., category-based, referral-based, etc.).
...
public:
    ...
    void applyDiscount(Cart& cart) const override {
        cout << "Applying Flat Discount: ₹" << flatAmount << endl;
...
public:
    ...
    void applyDiscount(Cart& cart) const override {
        ...
        cout << "Applying " << percent << "% Discount: ₹" << discount << endl;
...
public:
    ...
    void applyCoupons(Cart& cart, const vector<string>& couponIds) {
        ...
        cout << "\nInitial Cart Total: ₹" << before << endl;
        ...
        cout << "Final Cart Total: ₹" << cart.getTotal() << endl;
    }
};
...
| -------------------------- | ------------------------------------- | ---------- | ----------- |
| `CouponManager`            | Controls application logic            | ✅          | ⚙️          |
| `Coupon` Interface         | Defines contract for all coupon types | ✅          | —           |
| `FlatDiscountCoupon`       | Implements fixed discount logic       | ✅          | —           |
| `PercentageDiscountCoupon` | Implements percentage discount logic  | ✅          | —           |
| `ThreadSafeCouponRegistry` | Manages coupons concurrently          | ✅          | ✅           |
| `Cart`                     | Holds cart data and applies discounts | ✅          | —           |
