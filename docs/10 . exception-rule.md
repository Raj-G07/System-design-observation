# ⚠️ Exception Rule in Object-Oriented Design
...
- Don’t catch exceptions too early or too late.
- Catch them where meaningful recovery can be done.
  
```cpp title="catch-exceptions.cpp" linenums="1"
try {
    processPayment();
} catch (const NetworkError& e) {
    // ✅ Correct: recoverable at this layer
    retryConnection();
} catch (const std::exception& e) {
    // ✅ Log and terminate safely
    logError(e.what());
}
```

### 3. Don’t Swallow Exceptions
- Avoid empty catch blocks — they hide bugs and make debugging impossible.
  
```cpp title="without-swallow-exceptions.cpp" linenums="1"
try {
    performOperation();
} catch (...) {
    // ❌ Bad: hides the error
...
- ✅ Instead:
- 
```cpp title="with-swallow-exceptions.cpp" linenums="1"
catch (const std::exception& e) {
...
```cpp title="custom-exception.cpp" linenums="1"
...
- Strong Guarantee — No side effects if an exception occurs.
- No-Throw Guarantee — Function never throws exceptions.
  
```cpp title="exception-safety.cpp" linenums="1"
void safeFunction() noexcept {  // ✅ No-throw guarantee
