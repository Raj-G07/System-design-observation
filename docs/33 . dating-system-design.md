...
- Follows service-oriented design — each feature runs as a microservice.
...
!!! note ""
    ...
    subgraph Services["Backend Services"]
        ...
        C3[Matchmaking Service]
        C4[Chat Service]
        C5[Notification Service]
        C6[Recommendation Engine]
        C7[Admin Dashboard Service]
    end
...
## C++ Implementation
```cpp title="dating-site.cpp" linenums="1"
// dating_app.cpp
...
## 🔄 Data Flow (Example)
!!! example ""
    - User signs up via the frontend → request goes through API Gateway → Authentication Service → User Database.
    - User uploads a photo → User Profile Service calls AI Moderation API → if approved, stored in Media Storage.
    - User likes another profile → Matchmaking Service updates Match Database → if mutual, triggers Notification Service.
    - Matched users chat → messages stored in Chat Database → notifications pushed in real-time.
