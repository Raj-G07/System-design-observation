# 🧱 Importance of OOP in System Design

Object-Oriented Programming (OOP) is not just a coding paradigm — it’s a **design philosophy** that directly influences how we build **scalable, maintainable, and modular systems**.  
In system design, OOP acts as the **bridge** between **conceptual architecture (HLD)** and **practical implementation (LLD)**.

---

## 🧠 1. What is OOP in the Context of System Design?

In large-scale systems, OOP helps **model real-world entities** as interacting software objects — encapsulating both **data (state)** and **behavior (methods)**.  
...
## ⚙️ 2. Four Core Principles of OOP and Their Role in System Design

!!! details ""
    | Principle | Meaning | Role in System Design |
    |:------------|:----------|:----------------------|
    | **Encapsulation** | Binding data and behavior together into a single unit (class). | Hides implementation details; enables modular design where changes in one component don’t affect others. |
    ...
    | **Inheritance** | Reusing properties and methods of existing classes to create new ones. | Promotes code reuse across modules (e.g., `User` → `AdminUser` → `SuperAdmin`). |
    | **Polymorphism** | The ability to process objects differently based on their data type or class. | Enables flexible and extensible design — useful in plug-in architectures, message handlers, and API layers. |

---

## 🧩 3. How OOP Supports System Design Pillars
...
## 🧱 4. Example — Designing a Payment System
!!! example ""
    Let’s see how OOP shapes the system design for a **Payment Gateway**.
    
    ### **Without OOP**
    You might have:
    ```cpp title="without-oop.cpp" linenums="1"
    #include <iostream>
    using namespace std;
    ...
    ```
    Adding a new payment method means editing this function — not scalable or maintainable.
    
    ### **With OOP**
    We use abstraction, inheritance, and polymorphism to create an extensible design.
    ```cpp title="with-oop.cpp" linenums="1"
    #include <iostream>
    #include <memory>
    using namespace std;
...
## 🧐 6.Related Repo
!!! link
    - [Home Electricity & Furniture Relational Model](https://github.com/Raj-G07/Classes)
