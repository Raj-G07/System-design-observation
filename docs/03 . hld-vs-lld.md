...
System Design can be divided into two major stages — **High-Level Design (HLD)** and **Low-Level Design (LLD)**.  
...
!!! note ""
    ...
    |:--------|:----------------------------|:----------------------------|
    | **Definition** | The blueprint of the entire system — shows how components interact at a macro level. | The detailed plan of how each component or module will be implemented. |
...
## ⚙️ 3. Example — Designing a Ride-Sharing App (e.g., Uber)
!!! example ""
    | Step | **High-Level Design (HLD)** | **Low-Level Design (LLD)** |
    |:-----|:-----------------------------|:----------------------------|
    | **Architecture** | Client → API Gateway → Ride Service → Payment Service → Notification Service | Detailed classes for `Ride`, `Driver`, `Rider`, `Payment`, `Notification` |
    | **Data Flow** | Rider sends ride request → System finds nearby driver → Driver accepts → Payment processed → Notification sent | `RideMatchingService` implements `findNearestDriver()` using Haversine formula; `PaymentService` handles payment via strategy pattern |
...
> Think of **HLD** as the **architect’s blueprint** for a building — showing floors, rooms, and structure.  
>  
> Think of **LLD** as the **engineer’s detailed plan** — specifying materials, wiring, and plumbing.
...
!!! details ""
    ...
    | Example Question | “Design WhatsApp system.” | “Design the class structure for chat messages.” |
...
- HLD gives the *“big picture”* — how the system works as a whole.
- LLD gives the *“small picture”* — how each piece fits and functions internally.
