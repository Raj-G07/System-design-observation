...
C -->|Valid| D[Select Payment Gateway Provider]
...
F -->|Others| I[Dynamic Adapter via Plugin System]

%% ========== STANDARDIZED PAYMENT FLOW ==========
G --> J[Standardized Payment Interface]
H --> J
I --> J
...
L -->|Yes| M[Update Payment Status: SUCCESS]
L -->|No| N[Error Handling Module]

N --> O{Retry Mechanism Type?}
O -->|Immediate Retry| P[Retry Immediately (Max 3 times)]
O -->|Scheduled Retry| Q[Retry after fixed interval]
O -->|Exponential Backoff| R[Retry with increasing delay]
P -->|Success| M
Q -->|Success| M
R -->|Success| M
P -->|Fail| S[Mark Payment as FAILED]
Q -->|Fail| S
R -->|Fail| S

%% ========== RECURRING PAYMENT FLOW ==========
M --> T{Recurring Payment Enabled?}
T -->|Yes| U[Schedule Next Payment Cycle]
T -->|No| V[Notify User of Success]

U --> W[Trigger Recurring Payment Scheduler]
W --> X[Initiate Next Cycle via Payment Service]
X --> C

S --> V[Notify User of Failure or Retry Exhausted]

%% ========== USER NOTIFICATIONS ==========
V --> Y[Send Email/SMS/Push Notification]
Y --> Z[End]
```
