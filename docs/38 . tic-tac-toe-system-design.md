...
- The board should support variable dimensions (3×3, 4×4, NxN).
...
**2. Server Side**
   ...
   - **Game Controller:**
     Acts as the central manager that coordinates between all modules — processes user moves, communicates with the rules engine, and updates the game state.

   - **Rules Engine:**
     Handles move validation, win/draw detection, and supports extensibility for new rule sets.

   - **Board Manager:**
     Maintains the board state. Supports scalable configurations like 3×3, 4×4, or larger boards.
...
subgraph Client["🎮 Client Side (Mobile/Web App)"]
    UI["Game UI\n(Board, Score, Status)"]
